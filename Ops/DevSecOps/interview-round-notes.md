...
- **Provenance (where an artifact came from and how it was built) & signing:** SLSA framework, sign artifacts/images with **Sigstore/cosign**, generate an **SBOM** (Syft), verify signatures before deploy.
- **Least privilege (only the permissions needed) for CI:** short-lived OIDC tokens instead of long-lived secrets, scoped runner permissions, isolated ephemeral runners.
...
- **Build/CI:** SAST + SCA + secret scan gates that fail the build; ephemeral least-privilege (minimum required access) runners; verify dependencies.
...
- **Hygiene:** least privilege (only the permissions needed), mask/rotate secrets, scope to environments, restrict on PRs from forks, and scan for leaked secrets.
