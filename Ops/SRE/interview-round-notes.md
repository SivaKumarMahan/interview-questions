...
- **Other principles:** eliminate toil via automation, embrace blameless postmortems, cap toil (~50%), and monitor the **four golden signals** (latency, traffic, errors, saturation).
...
Talk about concrete scale (N clusters/services/regions) and challenges: config drift and standardization (solved with IaC + GitOps), observability at scale (cardinality, cost — Thanos/sampling), multi-team coordination and safe rollouts (progressive delivery), cost optimization, on-call/toil reduction, and reliability during upgrades/migrations. Pair each challenge with what you did about it.
