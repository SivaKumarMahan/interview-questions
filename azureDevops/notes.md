...
4. The pipeline builds a minimal container image, scans it, publishes the immutable (not changed after creation) digest to ACR, and records build provenance (where an artifact came from and how it was built).
...
Important investigation areas from this project were Terraform state and lock handling, ACR authentication, Azure service-connection identity and RBAC, Helm rendering and release history, Kubernetes Events, and rollout health.

The main design rule is **build once and promote the same immutable (not changed after creation) artifact** rather than rebuilding for each environment.
---