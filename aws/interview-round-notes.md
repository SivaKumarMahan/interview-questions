...
- Add **WAF** on CloudFront, least-privilege (minimum required access) IAM, and disable ACLs (bucket-owner-enforced). Only the static assets should be reachable — nothing writable to the public.
...
- **IAM:** least privilege (only the permissions needed), roles over long-lived keys, **IAM roles for service accounts (IRSA)** in EKS and **OIDC** for CI, MFA on humans, no wildcard `*` policies, use permission boundaries + SCPs (Organizations), rotate/avoid access keys, audit with Access Analyzer.
