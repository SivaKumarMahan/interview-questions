...
4. The build creates one immutable package or Docker image, generates an SBOM/provenance, signs it, and pushes it to **ECR** or another approved registry.
5. **Terraform** provisions infrastructure; **Ansible** configures machines when that model applies.
6. **Helm/Kustomize** manifests describe Kubernetes resources. A GitOps repository records the desired digest and **Argo CD/Flux** reconciles it to EKS/AKS/other clusters.
...
Build once and promote the same digest across environments. Keep environment configuration versioned and secrets external. Production uses protected identity/approval, progressive delivery, health and SLO gates, and independent rollback. **CI** proves artifact quality; **CD** controls promotion and verifies the real application.
