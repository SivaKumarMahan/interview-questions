...
- Best practice: grant least privilege, use groups not per-user rules, and audit `/etc/sudoers`.
...
Use the USE method (Utilization, Saturation, Errors) across CPU, memory, disk, network:
...
Correlate with recent deploys, cron jobs, traffic spikes. Resolve: scale out/up, fix the offending process, add limits, tune the workload.
