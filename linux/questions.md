...
Then account: `getent passwd`, lock/expiry (`passwd -S`, `chage -l`), shell/home, Allow/Deny/PAM, group, key match. SSH permissions normally home not writable by others, `.ssh` 700, `authorized_keys` 600 and correct owner; SELinux context restored if needed.
...
Test `bash script` helps distinguish execute bit/noexec from contents but is not final bypass. I grant via correct group/ACL/ownership and minimal directory traverse, not world-write/777. For SELinux restore appropriate label/policy, not disable SELinux.
...
For shared project directory, set group/setgid and default ACL:
...
For activity over time, I use `inotifywait -m /path/file` to see changes. If the file is opened and closed very quickly, Linux audit is more reliable:
...
Cron has a small environment and a different working directory, so I use absolute command/file paths, a valid shebang, executable permissions, and explicitly define required variables. A useful temporary diagnostic entry is:
...
The first line is the header, so `head -n 11` displays ten processes. For investigation I prefer explicit columns sorted by resident memory:
